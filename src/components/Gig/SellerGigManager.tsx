...
import { Eye, Package, Edit, Trash2, Loader2, ChevronLeft, ChevronRight, EyeOff } from 'lucide-react';
...
const SellerGigManager: React.FC = () => {
  ...
  const fetchGigs = async () => {
    ...
    try {
      ...
      if (response.data.error) {
        ...
        setTotalPages(Math.max(1, Math.ceil(response.data.pagination.totalPages)));
      }
    } catch (err) {
      console.error(err);
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await axios.delete(`http://localhost:5000/api/gigs/delete/${gigToDelete}`, {
        withCredentials: true
      });
  ...
  const handleHideGig = async (id: string) => {
    try {
      const response = await axios.patch(`http://localhost:5000/api/gigs/hidden/${id}`, {}, {
        withCredentials: true
      });
      
      if (response.data.error) {
        const errorMessage = response.data.message === 'Cannot hide gig with existing orders' 
        ...
        setGigs(gigs.map(gig => 
          gig._id === id 
            ? {...gig, status: response.data.gig.status}
            : gig
        ));
        toast.success(
      ...
      const error = err as AxiosError<ErrorResponse>;
      const errorMessage = error.response?.data?.message === 'Cannot hide gig with existing orders'
        ? 'Không thể ẩn dịch vụ khi đang có đơn hàng tồn tại'
                ...
                <tr key={gig._id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-3 py-4 text-sm text-gray-500 text-right">
                    <div className="flex items-center justify-center space-x-3">
                      ...
                      >
                        {gig.status === 'hidden' ? <Eye className="w-4 h-4 mr-1" /> : <EyeOff className="w-4 h-4 mr-1" />}
            ...
            <div key={gig._id} className="bg-white p-4 rounded-lg shadow space-y-3 w-full">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex space-x-2">
                  ...
                  >
                    {gig.status === 'hidden' ? <Eye className="w-3 h-3 mr-1" /> : <EyeOff className="w-3 h-3 mr-1" />}
                    {gig.status === 'hidden' ? 'Hiện' : 'Ẩn'}
                  </button>
                  <button
                    onClick={() => openDeleteModal(gig._id)}
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-8 rounded-lg shadow-lg max-w-md w-full mx-4">
            ...
            <p className="text-gray-600 mb-6">Bạn có chắc chắn muốn xóa dịch vụ này? Hành động này không thể hoàn tác.</p>
