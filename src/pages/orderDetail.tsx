        ...
        if (response.data.error === false) {
          ...
          if (orderData.gigId) {
            ...
            if (typeof orderData.gigId === 'object' && orderData.gigId._id) {
              orderData.gigInfo = {
                images: orderData.gigId.media ? orderData.gigId.media.map((item: {url: string}) => item.url || '') : [],
            ...
            // Nếu gigId là string (chỉ chứa ID)
              ...
              try {
                ...
                if (!gigResponse.data.error && gigResponse.data.gig) {
                  orderData.gigInfo = {
                    images: gigResponse.data.gig.media ? gigResponse.data.gig.media.map((item: {url: string}) => item.url || '') : [],
          ...
          if (orderData.sellerId && (!orderData.sellerInfo || !orderData.sellerInfo.avatar)) {
            try {
              ...
              if (!sellerResponse.data.error && sellerResponse.data.user) {
                ...
                orderData.sellerInfo = {
                  name: sellerResponse.data.user.name || sellerResponse.data.user.username || orderData.sellerInfo?.name,
                  avatar: sellerResponse.data.user.avatar || orderData.sellerInfo?.avatar
                };
              }
            } catch (sellerErr) {
            ...
            {loading ? (
              ...
              <div className="bg-white rounded-2xl shadow-xl overflow-hidden transition-all duration-300 hover:shadow-2xl">
                ...
                <div className="p-4 sm:p-6 md:p-8">
                  ...
                  {((order.gigInfo && order.gigInfo.images && order.gigInfo.images.length > 0) || order.sellerInfo) && (
                    <div className="mb-6 pb-6 border-b border-gray-200">
                      <div className="flex flex-col md:flex-row gap-6">
                        {order.gigInfo && order.gigInfo.images && order.gigInfo.images.length > 0 && (
                            ...
                            <div className="bg-gray-100 rounded-lg overflow-hidden">
                              <img 
                                ...
                                onError={(e) => {
                                  ...
                                  target.src = "https://via.placeholder.com/400x300?text=Không+có+ảnh";
                                }} 
                              />
                            </div>
                            {order.gigIdValue && (
                              ...
                              {order.sellerInfo.avatar ? (
                                ...
                                <div className="w-16 h-16 rounded-full bg-blue-100 flex items-center justify-center">
                                  <span className="text-xl font-medium text-blue-600">
                                    {order.sellerInfo.name ? order.sellerInfo.name.charAt(0).toUpperCase() : "U"}
                                  </span>
                                </div>
                              )}
                              <div>
                                ...
                                {order.sellerId && (
                                  <button 
                                    onClick={() => navigate(`/profile/${order.sellerId}`)}
                  ...
                  <div className="mt-6 pt-6 border-t border-gray-200 flex flex-wrap gap-3 justify-end">
                    ...
                    {(order.status === "completed" && order.isReviewed && order.reviewId && order.gigIdValue) && (
                      <button 
                        onClick={() => navigate(`/gig/${order.gigIdValue}?reviewId=${order.reviewId}`)}
