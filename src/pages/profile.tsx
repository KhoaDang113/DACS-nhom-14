...
export default function ProfilePage() {
  ...
  const { isCustomer, isFreelancer, isAdmin, isLoading: roleLoading } = useUserRole();
    ...
    try {
      ...
      if (userId) {
        ...
        try {
          ...
          const profileResponse = await axios.get(`http://localhost:5000/api/profile/user/${userId}`, {
            withCredentials: true
          });
          
          if (profileResponse.data && !profileResponse.data.error && profileResponse.data.data) {
            ...
            if (profileResponse.data.data.role === 'freelancer') {
              try {
                ...
                const allGigsResponse = await axios.get(`http://localhost:5000/api/gigs`, {
                  withCredentials: true
                });
                ...
                if (allGigsResponse.data && !allGigsResponse.data.error) {
                  ...
                  gigsData = allGigsResponse.data.gigs.filter(gig => {
                    // Hiển thị thông tin freelancerId để debug
                    console.log(`Gig ${gig._id} có freelancerId:`, gig.freelancerId);
                    
                    if (gig.freelancerId === userId) return true;
                    if (gig.freelancer && gig.freelancer._id === userId) return true;
                    if (gig.freelancer && gig.freelancer === userId) return true;
                  ...
                  console.log('Gigs của người dùng:', gigsData);
                }
              } catch (error) {
        ...
        if (userResponse.data && !userResponse.data.error) {
          ...
          if (userResponse.data.data.role === 'freelancer') {
            try {
              ...
              console.log('Không thể lấy dữ liệu gig của người dùng này', error);
            }
          }
      ...
      const response = await axios.put('http://localhost:5000/api/profile/update', 
        {
          ...
          education: userData.education,
          certificates: userData.certificates,
          fullName: userData.fullName || (userData.name || clerkUser?.firstName + ' ' + clerkUser?.lastName || 'User Name')
        },
        { withCredentials: true }
      );
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 md:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
              ...
              <div className="flex flex-col items-center text-center mb-4">
                ...
                <div className="flex flex-col items-center gap-2 mt-4">
                  ...
                  <div className="flex items-center text-gray-600">
                    ...
                    <span>Tham gia {userData.createdAt ? 
            ...
            <ProfileInfo 
              user={{
                description: userData.role === 'freelancer'
                  ? (userData.freelancerProfile?.description || userData.description || '') 
                  : (userData.description || ''),
                hardSkill: userData.role === 'freelancer'
                  ? (userData.freelancerProfile?.hardSkill || userData.hardSkill || '') 
                  : '',
                softSkill: userData.role === 'freelancer'
                  ? (userData.freelancerProfile?.softSkill || userData.softSkill || '') 
                  : '',
                languages: userData.role === 'freelancer'
                  ? (userData.freelancerProfile?.languages || userData.languages || '') 
                  : '',
                education: userData.role === 'freelancer'
                  ? (userData.freelancerProfile?.education || userData.education || '') 
                  : '',
                certificates: userData.role === 'freelancer'
                  ? (userData.freelancerProfile?.certificates || userData.certificates || '') 
                  : ''
              }}
              isCustomer={userData.role === 'customer'}
              onUpdateUser={!userData.isViewingOtherUser ? handleUpdateUser : undefined}
              fetchUserProfile={!userData.isViewingOtherUser ? fetchUserProfile : undefined}
