...
const GigDetailPage = () => {
  ...
  useEffect(() => {
    ...
    const setupMedia = async () => {
      try {
        ...
        const videoItems = gig.media.filter(item => 
  ...
  const fetchReviews = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/api/review/gig/${id}/frontend`);
      if (response.data && !response.data.error) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
            ...
            <div className="grid grid-cols-5 gap-2">
              {processedMedia.map((mediaItem, index) => (
                ...
                >
                  ...
                  <img
                    src={mediaItem.type === "image" ? mediaItem.url : (mediaItem.thumbnailUrl || mediaItem.url)}
                    alt={`${gig.title} - ${mediaItem.type === "video" ? "video" : "ảnh"} ${index + 1}`}
                    className="w-full h-full object-cover"
                  />
                </div>
              ))}
          ...
          <div className="bg-gray-50 p-6 rounded-lg mb-10">
            ...
            <button 
              className={`border border-gray-300 rounded-md px-4 py-2 text-gray-700 ${isLocked ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-100'} transition-colors`}
              disabled={isLocked}
            >
              {isLocked ? (
            ...
            <div className="border-t p-6 space-y-3">
              ...
              {!isSignedIn ? (
                ...
                <button 
                  ...
                  className={`text-gray-700 hover:text-gray-900 flex items-center justify-center gap-2 font-medium w-full ${isLocked ? 'opacity-50 cursor-not-allowed' : ''}`}
                  disabled={isLocked}
                >
                ...
                <div className="flex items-start gap-3">
                  <div className="text-orange-500 mt-1">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 3.813-1.874 2.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126ZM12 15.75h.007v.008H12v-.008Z" />
                    </svg>
                  </div>
                  <div className="text-sm text-orange-700">
                    <p className="font-medium">Tài khoản của bạn đã bị khóa</p>
                    <p>Bạn chỉ có thể xem thông tin dịch vụ. Để sử dụng đầy đủ các chức năng, vui lòng liên hệ quản trị viên.</p>
                  </div>
                </div>
              </div>
            )}
